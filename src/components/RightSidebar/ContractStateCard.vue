<template>
  <q-card flat>
    ...
    <q-card-section class="q-pt-none">
      ...
      <q-scroll-area v-else-if="data" style="height: 500px" class="row">
        <vue-json-pretty :data="data" />
      </q-scroll-area>
    </q-card-section>
    <q-separator />
    <q-card-actions align="left" class="bg-grey-2">
      <q-btn @click="fetch" icon="download" no-caps flat color="primary"
        >Fetch</q-btn
      >
    </q-card-actions>
  </q-card>
...
const fetchState = async () => {
  try {
    ...
    data.value = await blockchainStore.getSmartContractState(
...
const fetchSubState = async () => {
  try {
    ...
    data.value = await blockchainStore.getSmartContractSubState(
      props.contract.address,
      variableName.value,
